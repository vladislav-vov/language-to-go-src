 ...
 */
    ...
    (() => {
        ...
        (function() {
            ...
            if (spollersArray.length > 0) {
                ...
                if (spollersMedia.length > 0) {
                    ...
                    mediaQueries = mediaQueries.filter(((item, i, arr) => arr.indexOf(item) == i));
                    mediaQueries.forEach((breakpoint => {
                        const [mediaQuery, mediaBreakpoint, mediaType] = breakpoint.split(",");
                        const matchMedia = window.matchMedia(mediaQuery);
                        const spollersArray = breakpointsArray.filter((item => item.value === mediaBreakpoint && item.type === mediaType));
            ...
            function setSpollerAction(e) {
                ...
                if (target.hasAttribute("data-spoller") || target.closest("[data-spoller]")) {
                    ...
                    if (!spollersBlock.querySelectorAll("._slide").length) {
                        ...
                        isParentNode ? spollerTitle.parentNode.classList.toggle("_spoller-active") : spollerTitle.classList.toggle("_spoller-active");
        ...
        window.addEventListener("DOMContentLoaded", (() => {
            ...
            function initSlider({sliderSelector, paginationSelector, breakpoints = {}, loop = false}) {
                if (document.querySelector(sliderSelector)) new Swiper(sliderSelector, {
                    modules: [ Pagination ],
                    spaceBetween: 40,
                    slidesPerView: 1,
                    loop,
        ...
        const timer = (id, minutes) => {
            const getTimeRemaining = endtime => {
                const time = endtime - Date.parse(new Date), seconds = Math.floor(time / 1e3 % 60), minutes = Math.floor(time / 1e3 / 60 % 60);
                return {
                    total: time,
                    minutes,
                    seconds
            ...
            const setClock = (selector, endTime) => {
                const timer = document.querySelector(selector), minutesElement = timer.querySelector("#minutes"), secondsElement = timer.querySelector("#seconds"), timeInterval = setInterval(updateClock, 1e3);
            ...
            const endTime = currentTime.getTime() + minutes * 60 * 1e3;
            setClock(id, endTime);
        };
        const files_timer = timer;
        window.addEventListener("DOMContentLoaded", (() => {
            if (document.querySelector(".login-form-code__timer")) files_timer(".login-form-code__timer", 1);
            ...
            codeForm.addEventListener("input", (function(event) {
                ...
                if (event.inputType !== "deleteContentBackward" && target.tagName === "INPUT" && target.type === "number") {
            ...
            codeForm.addEventListener("keydown", (function(event) {
                const target = event.target;
                if (event.key === "Backspace" && target.tagName === "INPUT" && target.type === "number") if (target.value) target.value = ""; else {
                    const index = Array.from(codeInputs).indexOf(target);
        ...
        window.addEventListener("DOMContentLoaded", (() => {
            ...
            function adjustAsideHeight() {
                ...
                if (aside.getBoundingClientRect().bottom >= viewportHeight) aside.style.height = toRem(updatedHeight); else aside.style.height = `auto`;
            }
            function resize() {
                if (window.innerWidth >= tablet) adjustAsideHeight(); else if (window.innerWidth <= tablet) aside.style.height = `auto`;
            }
        ...
        window.addEventListener("DOMContentLoaded", (() => {
            ...
            if (phoneInputs.length) Inputmask({
        ...
        let formValidate = {
            ...
            addError(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.error) formRequiredItem.parentElement.insertAdjacentHTML("beforeend", `<div class="form-error">${formRequiredItem.dataset.error}</div>`);
            },
            removeError(formRequiredItem) {
                formRequiredItem.classList.remove("_form-error");
                formRequiredItem.parentElement.classList.remove("_form-error");
                if (formRequiredItem.parentElement.querySelector(".form-error")) formRequiredItem.parentElement.removeChild(formRequiredItem.parentElement.querySelector(".form-error"));
            },
            formClean(form) {
                form.reset();
                setTimeout((() => {
        ...
        class CookieConsent {
            ...
            init() {
                this.bindTriggers();
                try {
                    if (!this.hasConsented()) this.popup.classList.add(this.activePopupClass);
                    myScripts();
                } catch (e) {
